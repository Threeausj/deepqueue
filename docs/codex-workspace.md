...
连续的思考摘要、命令执行、文件变更、工具调用和带有进度标记的说明，会合并到「思考与执行过程」中，默认整段收起。摘要显示记录数量、执行状态和失败项数；展开后可逐条查看，并在底部「收起这段过程」。用户消息、最终回复和审批请求持续显示；旧版模型未标明阶段的回复也保持可见。手动展开状态在流式更新及当前对话刷新时保留；收起内容按需渲染，减少长对话的页面负担。
