...
连续的思考摘要、命令执行、文件变更、工具调用和带有进度标记的说明，会合并到「思考与执行过程」中，默认整段收起。摘要显示记录数量、执行状态和失败项数；展开后可逐条查看，并在底部「收起这段过程」。用户消息保持独立展示；最终回复和审批请求持续显示，旧版模型未标明阶段的回复也保持可见。手动展开状态在流式更新及当前对话刷新时保留；收起内容按需渲染，减少长对话的页面负担。

较长的用户输入（包括实验结束后的归档回传内容）默认显示前 6 行，右上角「展开全文」可查看完整内容；展开后顶部和底部均可收起，底部收起会回到该消息的开头。是否折叠按实际排版高度判断，调整窗口或展开右侧工具面板时会自动适配。手动展开状态在当前对话刷新及实时更新时保留；折叠仅影响网页显示，不截断发送给 Codex 的内容或会话记录。

输入框下方显示上下文 token 用量、模型返回的窗口上限和占用比例，点击可查看精确数量及累计消耗。上下文占用使用最近一次请求的 `last.totalTokens`，累计消耗单独使用 `total.totalTokens`；未发送输入不计入，模型未提供窗口上限时不显示百分比。统计由 `thread/tokenUsage/updated` 实时同步，并按服务器、任务缓存在当前连接内，刷新历史或重新打开页面可恢复；重新连接后等待 Codex 再次返回统计，不以字数估算或显示虚假的 0%。

Codex 自动压缩时，输入框旁显示「正在压缩上下文」，对话中出现独立的压缩记录，完成、失败和中断状态各自展示。它依据 `contextCompaction` 的 `item/started` / `item/completed` 生命周期和 `turn/completed` 更新，兼容旧版 `thread/compacted`。压缩记录始终独立于「思考与执行过程」；压缩没有百分比进度时只显示运行状态。历史记录无法确定正在执行的压缩状态、或连接中断时，会显示「压缩状态待同步」。事件定义参考 [官方 app-server 协议](https://learn.chatgpt.com/docs/app-server#trigger-thread-compaction)。
...
历史读取（包括 CLI 的 `codex read`）在 `thread.context` 中附带已收集的 `tokenUsage` 和最近一次 `compaction` 状态；尚无统计时为 `{}`。SSE 的 `deepqueue/context` 通知携带 `{threadId, context}`，与 HTTP 快照一致，客户端应按服务器和任务 ID 区分。
